		...
		<?php
			...
			while ($row = mysqli_fetch_assoc($rs_target1)) {
				...
				if($zone == 0) {
					...
					//echo $zone1_percent;
					$update_chart1 = "UPDATE `raiting_zone_1` SET `percent` = '$zone1_percent' WHERE `userid` = '$id'";
					$rs_update_chart1 = mysqli_query($conn,$update_chart1);
			...
			while ($row2 = mysqli_fetch_assoc($rs_target2)) {
				...
				if($zone2 == 0) {
					...
					$update_chart2 = "UPDATE `raiting_zone_2` SET `percent` = '$zone2_percent' WHERE `userid` = '$id'";
					$rs_update_chart2 = mysqli_query($conn,$update_chart2);
			...
			while ($row3 = mysqli_fetch_assoc($rs_target3)) {
				...
				if($zone3 == 0) {
					...
					$update_chart3 = "UPDATE `raiting_zone_3` SET `percent` = '$zone3_percent' WHERE `userid` = '$id'";
					$rs_update_chart3 = mysqli_query($conn,$update_chart3);

					/* Ако качените материали са повече от таргета, да показва 100%, а не 130% примерно*/
					if ($zone3_percent > 100)  {
						$zone3_percent = '100';
					}
					//echo "процент покритие: " . round($zone3_percent) . "%";
				}
			}
			echo "<br/>";
			while ($row4 = mysqli_fetch_assoc($rs_target4)) {
				...
				if($zone4 == 0) {
					...
					$update_chart4 = "UPDATE `raiting_zone_4` SET `percent` = '$zone4_percent' WHERE `userid` = '$id'";
					$rs_update_chart4 = mysqli_query($conn,$update_chart4);
			...
			while ($row5 = mysqli_fetch_assoc($rs_target5)) {
				...
				if($zone5 == 0) {
					...
					$update_chart5 = "UPDATE `raiting_zone_5` SET `percent` = '$zone5_percent' WHERE `userid` = '$id'";
					$rs_update_chart5 = mysqli_query($conn,$update_chart5);
			...
			$sum_percent = $zone1_percent + $zone2_percent + $zone3_percent + $zone4_percent + $zone5_percent;

			$update_chart_all = "UPDATE `raiting_zone_all` SET `percent` = '$sum_percent' WHERE `userid` = '$id'";
			$rs_update_chart_all = mysqli_query($conn,$update_chart_all);
		
		?>

			<img src="images/statistic.png" class="statistic" width="15%;" style="position: absolute; padding-top: 20%; margin-left: 10%;">
